...
- 全部控件集中在一条紧凑工具条上：记录、字段、筛选、分组、排序、行高以及保存操作。
- 在记录浮层的同一个候选列表中选择一条或多条记录，插件不设数量上限。勾选不改变候选位置，对比列按勾选顺序追加；可在对比表头拖动调整顺序，保存后生效。
- 在插件内本地进行筛选、排序、分组和字段显示控制，不会修改原生数据表视图。
- 查看各记录之间存在差异的字段数量，差异行会被标记，并可用「仅看差异」开关只保留这些字段。
- 在字段浮层中统一排列显示和隐藏字段，并独立设置换行，保存后共享生效。拖动把手或使用 Alt＋上/下方向键排序；恢复默认顺序会沿用适配器顺序。
- 可在四档最小行高之间切换。文本展开会立即展开整个字段行，并可收起；只读用户也能使用，无需保存。
- 字段列与每个记录列可独立调宽，拖动即时生效并自动共享；失败保留本地宽度，支持重试和恢复，不提交其他草稿。
- 附件以 240px 高的区域显示当前大图，单元格与只读图库支持同步切换；不可用图片及其他附件保留文件名。
- 点击保存后才应用记录和字段草稿；列宽独立自动共享，不提交这些草稿。
