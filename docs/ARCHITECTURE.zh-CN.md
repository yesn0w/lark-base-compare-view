...
`useFieldValues` 会按需加载查询控制所需的原始字段值。`queryEngine` 是纯函数：规范化值、筛选、稳定排序，并让一条记录最多进入一个分组。`deriveCandidateRecords` 只根据查询规则推导候选顺序，不依赖勾选顺序或对比列顺序。`useCellValues` 加载结构化矩阵值，其中包含稳定的显示文本以及仅存于内存的附件呈现元数据。
