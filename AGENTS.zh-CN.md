...
- Bridge 数据只可保存所选记录 ID、隐藏字段、本地查询条件、排序规则、分组字段、字段顺序、换行字段 ID 和列宽；不得持久化语言、主题、分组折叠、临时行展开或附件图片位置。
- 记录、查询和字段改动先保留为草稿。比较矩阵使用最后一次成功保存的配置，列宽即时预览并自动共享是例外。临时行展开是立即生效的只读展示操作，不得将共享配置标记为未保存。
- 列宽自动共享不提交其他草稿。所有配置写入必须串行；失败保留待共享列宽并提供重试/恢复。
- 支持选择一条或多条记录，插件不设数量上限；支持本地筛选/分组/排序、横向滚动、字段可见性及对比列手动排序。勾选不得改变候选顺序。
- 每轮比较单元格加载按最多 12 个任务分批执行，不得持久化单元格值或静默截断已选 ID。
